...
export const loginUser = async(userData:SigninSchema)=>{
  ...
  const accessToken = generateToken(payload,process.env.ACCESS_SECRET as string,'30m')
  const refreshToken = generateToken(payload,process.env.REFRESH_SECRET as string,'7d')
